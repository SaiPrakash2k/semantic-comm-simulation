...
# --- State: [cpu_load, mem_load, data_size, last_noise, last_bandwidth] ---
            ...
            try:
                ...
                with conn:
                    ...
                    if not data or len(data) != 12:
                        print(f"[Feedback] Received incomplete data (got {len(data)} bytes).")
                        continue
...
def get_local_state():
    ...
    data_size = np.random.uniform(STATE_SPACE_LOW[2], STATE_SPACE_HIGH[2])
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        for step in range(1, TOTAL_TRAINING_STEPS + 1):
            ...
            if action == ACTION_SEMANTIC:
                ...
                message_dict['payload'] = get_image_feature_vector(image_path)
            ...
            # --- NEW: Serialize the dict using pickle ---
            message_payload = pickle.dumps(message_dict)

            print(f"Step {step}: State={np.round(state, 2)}")
            print(f"  -> Action: {log_msg_type}")
            send_message(s, message_payload) # Send the pickled bytes
            
            try:
                reward, noise, bandwidth = feedback_queue.get(timeout=20.0)
            except queue.Empty:
